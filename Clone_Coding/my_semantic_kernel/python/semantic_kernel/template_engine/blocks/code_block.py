...
class CodeBlock(Block, CodeRenderer):
    ...
    def is_valid(self) -> Tuple[bool, str]:
        ...
        if len(self._tokens) > 1:
            if self._token[0].type != BlockTypes.FUNCTION_ID:
                error_msg = f"Unexpected second token found: {self._tokens[1].content}"
                self.log.error(error_msg)
                return False, error_msg
            
            if (
                self._token[1].type != BlockTypes.VALUE
    ...
    async def render_code_async(self, context: SKContext):
    ...
    async def _render_function_call_async(self, f_block: FunctionIdBlock, context: SKContext):
