...
class PromptTemplateEngine(PromptTemplatingEngine):
    ...
    def extract_blocks(
        ...
        self._logger.debug(f"Extraction blocks from template: {template_text}")
    ...
    async def render_code_aync(
        ...
        for block in blocks:
            ...
            if not isinstance(block, CodeRenderer):
                raise ValueError("CodeBlock must implement CodeRenderer proto")
